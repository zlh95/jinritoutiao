...
3.对象可迭代的原因，是因为满足了迭代协议，即某个对象实现了__iter__()方法，我们认为它是可迭代的，如果没有实现__iter__()方法，
  但是有__getitem__()时，python会调用它，传入0开始的索引，尝试迭代对象，这是后备机制，虽然没有实现__contains__(),
  但也是能使用‘in’运算符，和迭代.
...
class Sentence:
	...
	def __getitem__(self,index): #有__getitem__()时，python会调用它，传入0开始的索引，尝试迭代对象，这是后备机制，虽然没有实现__contains__(),但也是能使用‘in’运算符，和迭代
