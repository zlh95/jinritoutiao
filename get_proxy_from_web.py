...
def get_proxys(page=1):
    #requests的session()可以自动保持cookies，不需要自己维护cookie内容
    s = requests.session()
    #高匿代理网站
    ...
    #utf编码
    #print(target_response.apparent_encoding)
...
def reg():
    #匹配丢包数
    lose_time = re.compile('丢失 = (\d+)',re.IGNORECASE)
    #匹配平均时间
    waste_time = re.compile('平均 = (\d+)',re.IGNORECASE)
    return lose_time,waste_time

if __name__ == '__main__':
    #初始化正则表达式
    lose_time,waste_time = reg()
    #获取ip代理
